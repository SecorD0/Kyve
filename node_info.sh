...
while test $# -gt 0; do
	...
	-h|--help)
		...
		echo
		echo -e "${C_LGn}Functionality${RES}: the script shows information about a Kyve node"
		echo
		...
		echo -e "https://github.com/SecorD0/Kyve/blob/main/node_info.sh - script URL"
...
token_name="kyve"
node_dir="$HOME/.kyve/"
wallet_name="$kyve_wallet_name"
wallet_address="$kyve_wallet_address"
wallet_address_variable="kyve_wallet_address"
validator_address="$kyve_validator_address"
validator_address_variable="kyve_validator_address"
...
if [ "$network" == "mainnet" ]; then
	...
	global_rpc="https://api.explorer.kyve.network/blocks/latest"
	explorer_url_template="https://explorer.kyve.network/korellia/staking/"
...
main() {
	...
	# Send Pull request with new texts to add a language - https://github.com/SecorD0/Kyve/blob/main/node_info.sh
	#elif [ "$language" = ".." ]; then
	else
	...
	if [ -n "$wallet_name" ] && ([ ! -n "$wallet_address" ] || [ ! -n "$validator_address" ]); then
		...
		local password
		read password
		local wallet_address=`echo "$password" | $daemon keys show "$wallet_name" --keyring-backend "$keyring_backend" -a`
	...
	local node_info=`$daemon query staking validators --node "$local_rpc" --limit 10000 -oj 2>/dev/null | jq -r '.validators[] | select(.operator_address=='\"$validator_address\"')'`	
